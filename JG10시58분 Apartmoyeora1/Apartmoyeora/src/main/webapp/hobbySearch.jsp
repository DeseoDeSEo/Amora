		...
		<div>
			...
			<div class="gtco-container">
				<div class="row">
					...
					<!--  <form action ="writee" method="post" > -->
						<div class="board_wrap">
							<div class="board_title">
								...
								<div class="row">
									<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				                        ...
				                        BoardDAO dao = new BoardDAO();
											ArrayList<BoardDTO> list_Search = new ArrayList<>();
													if("title_content".equals(col)){list_Search = dao.getListHobby_Search_Title_Content(sentence, pageNumber);}
													else if("content".equals(col)){list_Search =dao.getListHobby_Search_Content(sentence, pageNumber);}
													else if("title".equals(col)){list_Search =dao.getListHobby_Search_Title(sentence, pageNumber);}
													else if("writer".equals(col)){list_Search = dao.getListHobby_Search_Writer(sentence, pageNumber);}
								...
								<%-- 페이지 개수 숫자 나오는 곳	<% 
									...
									%>
										<a style ="border-color: #7297F7; background-color: #7297F7;" href="community.jsp?pageNumber=<%=pageNumber + 1%>" class="btn btn-success btn-arrow-left">다음</a>
									<%
										}
									%> 
									<%
													int n = (int)(dao.getCount() / 10 + 1);
													for (int i = 1; i <= n; i++) {
									%>
													<a href="community.jsp?pageNumber=<%=i%>">|<%=i%>|</a>
									<% } %>  --%>
							...
							<!-- 검색 버튼임. -->
								<div style="position:relative; left:40px; font-size:20px; margin-top: 20px; text-align: center; height: 25px;">
									<form action="hobbySearch.jsp" method="get">
										<select name="col">
