...
<body>
	...
	<div id="page">
		...
		<header id="gtco-header" class="gtco-cover gtco-cover-sm"
		    ...
		    <p>공지사항</p>
			<p>아모라 아파트의 공지사항은 빠르고 정확하게 안내드립니다.</p>
<!-- 이종권 여기시작 -->
        ...
        <table class="table table-striped" style="text-align: center; border: 1px solide #dddddd">
           ...
           <tbody>
		  		...
		  		<tr>                
					...
					<td><a href="ntc_view.jsp?no_num=<%=list.get(i).getNo_num()%>"><%=list.get(i).getNo_title().replaceAll(" ","&nbsp;").replaceAll("<","&lt;").replaceAll("<","&gt;").replaceAll("\n","<br>")%></a></td>
					<td><%= list.get(i).getId()%></td>
					<td><%=	list.get(i).getNo_date().substring(0,11) + list.get(i).getNo_date().substring(11, 13) + "시" + list.get(i).getNo_date().substring(14,16) + "분" %></td>	                             			
               </tr>
