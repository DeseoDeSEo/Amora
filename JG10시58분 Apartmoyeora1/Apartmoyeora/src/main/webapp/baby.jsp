						...
						<div class="board_wrap">
								...
								<!-- 글 출력되는 카테고리 란 -->
									<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
										...
										//출력되는 곳
												...
												ArrayList<BoardDTO> list = boardDAO.getListBaby(pageNumber);							
									...
									<% 
									//페이지 이동하는 버튼: 이전 다음.
									    System.out.println(boardDAO.nextPageRequest(pageNumber + 1));
										//page개수가 1개이면 페이지 이동 버튼 안 뜨도록 ! 
										//page개수가 1개 이상이면 페이지 이동 버튼이 나옴.
									
										if (pageNumber != 1) {
									%>
										<a href="baby.jsp?pageNumber=<%=pageNumber - 1%>" class="btn btn-success btn-arrow-left">이전</a>
									<%
										} if (boardDAO.nextPageBaby(pageNumber + 1)) {
									%>
										<a style ="border-color: #7297F7; background-color: #7297F7;" href="baby.jsp?pageNumber=<%=pageNumber + 1%>" class="btn btn-success btn-arrow-left">다음</a>
									<%
										}
									%> 
									<%
													int n = (int)(boardDAO.getCountBaby() / 10 + 1);
													for (int i = 1; i <= n; i++) {
									%>
													<a href="baby.jsp?pageNumber=<%=i%>">|<%=i%>|
													</a>
									<%
														}
													String col ="";
							...
							<!-- 검색 버튼 임. -->
								<div style="position:relative; left:40px; font-size:20px; margin-top: 20px; text-align: center; height: 25px;">
									<!-- 검색 누르면 search로 이동함.   -->
									<form action="babySearch.jsp" method="get">
										<select name="col">
