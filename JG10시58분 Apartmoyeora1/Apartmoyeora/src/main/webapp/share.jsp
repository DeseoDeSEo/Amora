...
<style>
    ...
    .ad-num {
        display: table-cell; /* or any other desired display value */
    }
</style>
						...
						<div style="margin-top: 10px;" class="board_wrap">
							...
							<div class="board_list_wrap" style="text-align: center;">
								<div class="board_list">
										...
										<div class="container px-4 px-lg-5 mt-5">
											<div
												...
												<%
												    ...
												    ArrayList<ShDTO> list = shDAO.getList(pageNumber);
												...
												<div class="col mb-5">
												    <div class="card h-100">
												    	<%
														ShDAO dao = new ShDAO();
														String imagePath = dao.sh_img();
														%>
												        <!-- Product image-->
												        <%-- <img class="card-img-top" src="<%= ad.getImageURL() %>" alt="..." /> --%>
														<td class="hidden-cell sh-num"><%= list.get(i).getSh_Num() %></td>												        
														<img class="card-img-top" src="${pageContext.request.contextPath}/sh_img/apple.jpg" alt="..." />
												        <!-- Product details-->
												        <div class="card-body p-4">
												            <div class="text-center">
												                <!-- Product name (AD_TITLE)-->
												                <h5 class="fw-bolder"><%= ad.getSh_title() %></h5>
												        ...
												        <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
												            <div class="text-center">
																<a class="btn btn-outline-dark mt-auto" href="share_view.jsp?sh_Num=<%= list.get(i).getSh_Num() %>">상세 보기</a>
												            </div>
												        </div>
												    </div>
												</div>
									...
									<% 
									    System.out.println(shDAO.nextPage(pageNumber + 1));
									    if (pageNumber != 1) {
									%>
									    <a href="share.jsp?pageNumber=<%=pageNumber - 1%>" class="btn btn-success btn-arrow-left">이전</a>
									<%
									    }
									%>
									
									<%
									    int n = (int)(shDAO.getCount() / 8 + 1);
									    for (int i = 1; i <= n; i++) {
									%>
									    <a href="share.jsp?pageNumber=<%=i%>" class="<%= i == pageNumber ? "page-number current-page" : "page-number" %>"><%=i%></a>
									<%
									    }
									%>
									
									<%
									    if (shDAO.nextPage(pageNumber + 1)) {
									%>
									    <a href="share.jsp?pageNumber=<%=pageNumber + 1%>" class="btn btn-success btn-arrow-right">다음</a>
									<%
									    }
									%>
