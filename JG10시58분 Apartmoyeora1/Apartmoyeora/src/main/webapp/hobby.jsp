					...
					<div class="topmargin">
						...
						<div class="board_wrap">
							<div class="board_title">
								<h1 style ="font-size:40px;"><b>취미생활</b></h1>
								...
								<!-- 글 출력되는 카테고리 란 -->
									<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
										...
										//출력되는 곳
												...
												ArrayList<BoardDTO> list = boardDAO.getListHobby(pageNumber);							
												for (int i = 0; i < list.size(); i++) { %>
											<tr style ="background-color:white;">
												<td><%= list.get(i).getCom_Category() %></td>
												<td><%= list.get(i).getCom_Num() %></td>
												<td><a href="view.jsp?com_Num=<%= list.get(i).getCom_Num() %>"><%= list.get(i).getCom_Title() %></a></td>
									...
									<% 
									//페이지 이동하는 버튼: 이전 다음.
									    System.out.println(boardDAO.nextPageHobby(pageNumber + 1));
										//page개수가 1개이면 페이지 이동 버튼 안 뜨도록 ! 
										//page개수가 1개 이상이면 페이지 이동 버튼이 나옴.
									
										if (pageNumber != 1) {
									%>
										<a href="hobby.jsp?pageNumber=<%=pageNumber - 1%>" class="btn btn-success btn-arrow-left">이전</a>
									<%
										} if (boardDAO.nextPageHobby(pageNumber + 1)) {
									%>
										<a style ="border-color: #7297F7; background-color: #7297F7;" href="hobby.jsp?pageNumber=<%=pageNumber + 1%>" class="btn btn-success btn-arrow-left">다음</a>
									<%
										}
									%> 
									<%
													int n = (int)(boardDAO.getCountHobby() / 10 + 1);
													for (int i = 1; i <= n; i++) {
									%>
													<a href="hobby.jsp?pageNumber=<%=i%>">|<%=i%>|
													</a>
									<%
														}
													String col ="";
