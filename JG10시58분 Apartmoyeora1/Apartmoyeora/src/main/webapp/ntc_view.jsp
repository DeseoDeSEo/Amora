...
<!-- 이종권 여기시작 -->
	...
	<div class="container">
		<div class="row">
			...
			<!-- table-striped 클래스는 Bootstrap에서 제공하는 스타일 클래스 중 하나로, 테이블의 행을 번갈아가며 서로 다른 배경 색으로 스트라이프 모양으로 스타일링합니다.  -->
		  	<table class="table table-striped" style="text-align:center; border:1px solid #dddddd">
		  		...
		  		<tbody>
		  			...
		  			<tr>
		  				<td >작성자</td>
		  				<td colspan="2" ><%= bbs.getId().replaceAll(" ","&nbsp;").replaceAll("<","&lt;").replaceAll(">", "&gt;").replaceAll("\n","<br>") %></td>
		  			</tr>
		  			<tr>
		  				<td >작성일자</td>
			...
			<a href= "pricing.jsp" class="btn btn-primary">목록</a> 
			<%
			...
			%>	  
				<a href="ntc_update.jsp?bbsID=<%=no_num %>" class="btn btn-primary">수정</a>
				
                <!--onclick="return confirm('정말로 삭제하시겠습니까?')" 버튼을 통해서 삭제를 메세지를 확인한다  -->
                <a onclick="return confirm('정말로 삭제하시겠습니까?')" href="ntc_deleteAction.jsp?bbsID=<%=no_num %>" class="btn btn-primary">삭제</a>
			<% 	  
			  }
			%>
